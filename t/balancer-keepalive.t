...
=== TEST 1: upstream_keepalive_module pooling: sanity
--- http_upstream
...
--- http_upstream
    upstream test_upstream {
        ...
        balancer_by_lua_block {
            ...
            if ngx.var.arg_sni == "three" then
                pool = ip .. ":" .. port
                ngx.log(ngx.DEBUG, "lua balancer: keepalive custom cpool: ", pool)
            end
...
=== TEST 4: enable_keepalive: sanity (static pool)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
lua balancer: keepalive reusing connection \S+, requests: 2, cpool: \S+
lua balancer: keepalive saving connection \S+, cpool: \S+, connections: 1
lua balancer: keepalive reusing connection \S+, requests: 3, cpool: \S+
lua balancer: keepalive saving connection \S+, cpool: \S+, connections: 1
lua balancer: keepalive reusing connection \S+, requests: 4, cpool: \S+
lua balancer: keepalive saving connection \S+, cpool: \S+, connections: 1$/



=== TEST 5: enable_keepalive: sanity (dynamic pool option)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 6: upstream_keepalive_module pooling: no granular pooling (TLS properties)
--- http_upstream
    upstream test_upstream {
        server 127.0.0.1:$TEST_NGINX_SERVER_SSL_PORT;
        keepalive 60;
...
--- config
    ...
    location ~ ^/client_one/tls12/proxy/(?<upstream_uri>.*) {
        ...
        proxy_ssl_certificate     $TEST_NGINX_CERT_DIR/test.crt;
        proxy_ssl_certificate_key $TEST_NGINX_CERT_DIR/test.key;
    ...
    location ~ ^/client_two/tls12/proxy/(?<upstream_uri>.*) {
        proxy_ssl_protocols       TLSv1.2;
        proxy_ssl_certificate     $TEST_NGINX_CERT_DIR/test2.crt;
        proxy_ssl_certificate_key $TEST_NGINX_CERT_DIR/test2.key;
...
--- response_body_like
ssl_client_s_dn=.*?CN=test\.com.*? ssl_protocol=TLSv1\.2
ssl_client_s_dn=.*?CN=test\.com.*? ssl_protocol=TLSv1\.2
ssl_client_s_dn=.*?CN=test\.com.*? ssl_protocol=TLSv1\.2
ssl_client_s_dn=.*?CN=test\.com.*? ssl_protocol=TLSv1\.2
--- grep_error_log_out
...
=== TEST 7: enable_keepalive: granular pooling (TLS properties)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
        ...
        balancer_by_lua_block {
            ...
            local ok, err = b.set_current_peer("127.0.0.1", $TEST_NGINX_SERVER_SSL_PORT, {
                pool = ngx.var.client_cert .. ngx.var.client_protocol,
            })
            if not ok then
                ngx.log(ngx.ERR, "failed to set current peer: ", err)
                return
...
=== TEST 8: set_current_peer: bad argument #3 'opts'
--- config
    location = /t {
...
=== TEST 9: set_current_peer: bad option 'pool'
--- config
    location = /t {
        content_by_lua_block {
            local b = require "ngx.balancer"
...
=== TEST 10: set_current_peer: bad option 'pool_size'
--- config
    location = /t {
        content_by_lua_block {
            local b = require "ngx.balancer"
...
=== TEST 11: enable_keepalive: bad usage (no upstream)
--- config
    location = /t {
        content_by_lua_block {
            local b = require "ngx.balancer"
...
=== TEST 12: enable_keepalive: bad usage (bad context)
--- http_upstream
    upstream test_upstream {
        server 127.0.0.1:$TEST_NGINX_SERVER_SSL_PORT;
    }
...
=== TEST 13: enable_keepalive: bad usage (no current peer)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 14: enable_keepalive: bad argument #1 'idle_timeout'
--- config
    location = /t {
        content_by_lua_block {
            local b = require "ngx.balancer"
...
=== TEST 15: enable_keepalive: bad argument #2 'max_requests'
--- config
    location = /t {
        content_by_lua_block {
            local b = require "ngx.balancer"
...
=== TEST 16: enable_keepalive: reaching 'max_requests > 0' closes keepalive connection (and frees empty pool)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 17: enable_keepalive: 'max_requests == 100' is the default value
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 18: enable_keepalive: 'max_requests == 0' never closes upstream connections
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 19: enable_keepalive: reaching 'idle_timeout > 0' closes keepalive connection (and frees empty pool)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 20: enable_keepalive: reaching 'pool_size' closes extra connections
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
lua balancer: keepalive no free connection, cpool: \S+
lua balancer: keepalive no free connection, cpool: \S+
lua balancer: keepalive saving connection \S+, cpool: \S+, connections: 3
lua balancer: keepalive saving connection \S+, cpool: \S+, connections: 3
lua balancer: keepalive closing connection \S+, cpool: \S+, connections: 2
...
=== TEST 21: enable_keepalive: keepalive connections can be closed by upstream (and frees empty pools)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 22: enable_keepalive: does not save non-keepalive connections (and frees empty pools)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 23: enable_keepalive: does not save bad connections (and frees empty pools)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 24: enable_keepalive: does not save timed out connections (and frees empty pools)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 25: enable_keepalive: can be called again upon retry (and frees empty pools)
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 26: enable_keepalive: must be called for each try
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
...
=== TEST 27: enable_keepalive: bypasses previous 'keepalive' directive
--- http_upstream
    upstream test_upstream {
        server 0.0.0.1;
        keepalive 10;
...
=== TEST 28: enable_keepalive: is superseded by a subsequent 'keepalive' directive
