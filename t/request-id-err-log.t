...
=== TEST 1: api_request_id in error log set
--- config
    location /t {
        set $api_request_id 1234;
        lua_error_log_request_id $api_request_id;
...
=== TEST 2: api_request_id in error log set when a runtime error occurs
--- config
    location /t {
        set $api_request_id 1234;
        lua_error_log_request_id $api_request_id;
...
--- config
    location = /append_method {
        set $api_request_id_b 654321;
        lua_error_log_request_id $api_request_id_b;
        content_by_lua_block {
            ngx.log(ngx.INFO, "log_msg")
            ngx.exit(200)
        }
    ...
    location = /append_req_id {
        set $api_request_id_a 123456;
        lua_error_log_request_id $api_request_id_a;
        content_by_lua_block {
            ngx.log(ngx.INFO, "log_msg")
            ngx.exit(200)
        }
...
--- http_config
    lua_error_log_request_id $api_request_id_global;
--- config
    set $api_request_id_global global;
    set $api_request_id_local local;

    location = /test {
        lua_error_log_request_id $api_request_id_local;
        content_by_lua_block {
            ngx.log(ngx.INFO, "log_msg")
            ngx.exit(200)
        }
