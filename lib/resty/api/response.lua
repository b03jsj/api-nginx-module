...
function _M.set_gzip(opts)
    local r = get_request()
    local rc = C.ngx_http_api_set_gzip(r, opts.buffer_num, opts.buffer_size, opts.compress_level)
...
function _M.skip_header_filter_by_lua()
    local r = get_request()
    local rc = C.ngx_http_api_skip_header_filter_by_lua(r)
    if rc == NGX_ERROR then
        return nil, "no memory"
    end
    return true
...
function _M.skip_body_filter_by_lua()
    local r = get_request()
    local rc = C.ngx_http_api_skip_body_filter_by_lua(r)
    if rc == NGX_ERROR then
        return nil, "no memory"
    end
    return true
