...
ngx_int_t
ngx_http_api_client_set_max_body_size(ngx_http_request_t *r, off_t bytes);
...
function _M.set_client_max_body_size(bytes)
    local r = get_request()
    local ret = C.ngx_http_api_client_set_max_body_size(r, tonumber(bytes))
...
function _M.enable_mirror()
    local r = get_request()
    local ret = C.ngx_http_api_enable_mirror(r)
    if ret == NGX_ERROR then
...
function _M.set_real_ip(ip, port)
    ...
    local r = get_request()
    local rc = C.ngx_http_api_set_real_ip(r, ip, #ip, port)
    if rc ~= NGX_OK then
...
function _M.set_proxy_request_buffering(on)
    local r = get_request()
    local ret = C.ngx_http_api_set_proxy_request_buffering(r, on and 1 or 0)
    if ret == NGX_ERROR then
